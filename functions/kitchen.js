...
const recipeDirectory = path.join(process.cwd(), 'content/kitchen');
...
const fetchRecipeContent = async () => {
  ...
  const allContent = await Promise.all(recipeFileNames.map(filename => import(`../content/kitchen/${filename}`)));
